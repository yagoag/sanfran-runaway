...
Você está em São Francisco, a cidade com o maior número de hipsters por metro quadrado do mundo, e todos sabem que não tem nada que um hipster goste mais do que um bom café.
...
Para rodar os testes em modo de monitoramento (serão re-executados ao alterar os arquivos), basta rodar `yarn test`
...
A aplicação também possui um processo de build & deployment automatizado na Netlify. Ele roda após todo commit enviado para a `master`.

Para ver os deployments, [clique aqui](https://app.netlify.com/sites/sanfranrunaway/deploys) ou no badge de build status na parte superior deste Readme.
